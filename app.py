...
def deck_draw(grid, title, is_final_deck):
	...
	while not done:
		...
		if is_final_deck:
			...
			pane2.addText(str(FINAL_ANSWER), RED, (300, 200))
			print(FINAL_ANSWER)
			time.sleep(10)
		...
		if not is_final_deck:
			...
			if 'yes' in answer:
				...
				ans = -1
				number_of_tries += 1
				question_text = 'Please try saying Yes or No again, Number of voice tries count :- '+str(number_of_tries)
				pane1.addText(question_text, GREEN, (2+SCREEN_SHIFT_X, 520+grid_text_vertical_position_start+SCREEN_SHIFT_Y))
				question_text = "Please Say 'Yes' if Your Number is in the Deck Shown or Say 'No'"
				pane1.addText(question_text, GREEN, (2+SCREEN_SHIFT_X, 570+grid_text_vertical_position_start+SCREEN_SHIFT_Y))
